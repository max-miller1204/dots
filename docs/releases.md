...
smoke-tests `dots-X.Y.Z.tar.gz`, then publishes that archive, `SHA256SUMS`,
and the updater-readable `dots-release.txt` manifest on the GitHub release. The
...
committed `HEAD`, so uncommitted files cannot leak into a release. Verify a downloaded
archive with `sha256sum -c SHA256SUMS` on Linux or
