...
| `~/.local/state/dots/update-terminal-status` | Out-of-band terminal sidecar written by the locked outer stage only after `script(1)` returns and closes the complete, incomplete, failed-migration, or deliberately aborted transcript. Hook and migration transcript text cannot forge completion. |
