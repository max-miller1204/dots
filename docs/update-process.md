...
| `~/.local/state/dots/update-orderly-exit` | Temporary status evidence atomically published by an orderly transcribed child. The locked outer stage removes stale evidence before recording and accepts it only when it matches `script(1)`'s status. It is never terminal evidence by itself. |
